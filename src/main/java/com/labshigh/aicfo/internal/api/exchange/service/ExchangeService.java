package com.labshigh.aicfo.internal.api.exchange.service;
...
import com.labshigh.aicfo.internal.api.exchange.dao.ExchangeVirtual2Dao;
import com.labshigh.aicfo.internal.api.exchange.dao.ExchangeVirtualDao;
import com.labshigh.aicfo.internal.api.exchange.mapper.ExchangeMapper;
import com.labshigh.aicfo.internal.api.exchange.mapper.ExchangeVirtual2Mapper;
import com.labshigh.aicfo.internal.api.exchange.mapper.ExchangeVirtualMapper;
...
import com.labshigh.aicfo.internal.api.exchange.model.response.ExchangeResponseModel;
import com.labshigh.aicfo.internal.api.exchange.model.response.ExchangeVirtualListResponseModel;
import com.labshigh.aicfo.internal.api.exchange.model.response.ExchangeVirtualResponseModel;
...
public class ExchangeService {
  ...
  public ExchangeVirtualResponseModel getVirtual(ExchangeGetVirtualRequestModel requestModel) {
    ...
    if (requestModel.getExchangeName().equals("FOGnet_USD")) {
  ...
  public List<ExchangeVirtualListResponseModel> getList(ExchangeListGetRequestModel exchangeListGetRequestModel) {

    List<ExchangeVirtualDao> exchangeVirtualDaoList = exchangeVirtualMapper.getList(exchangeListGetRequestModel);
    List<ExchangeVirtualListResponseModel> exchangeVirtualListResponseModelList = new ArrayList<>();
    if (exchangeVirtualDaoList == null) {
      throw new ServiceException(String.format(Constants.MSG_NO_DATA));
    }
    exchangeVirtualDaoList.forEach(exchangeVirtualDao -> {
      Double change = Double.parseDouble(exchangeVirtualDao.getLast()) -Double.parseDouble(exchangeVirtualDao.getPreviousClose());
      String mark = "flatten";
      if( change > 0 ){
        mark ="rise";
      }else if(change < 0 ){
      ...
      ExchangeVirtualListResponseModel exchangeVirtualListResponseModel =
        ExchangeVirtualListResponseModel.builder()
          .uid(exchangeVirtualDao.getUid())
          .createdAt(exchangeVirtualDao.getCreatedAt())
          .updatedAt(exchangeVirtualDao.getUpdatedAt())
          .deletedFlag(exchangeVirtualDao.isDeletedFlag())
          ...
          .change(mark)
          .build();
      exchangeVirtualListResponseModelList.add(exchangeVirtualListResponseModel);
    });

    return exchangeVirtualListResponseModelList;
  }
}
