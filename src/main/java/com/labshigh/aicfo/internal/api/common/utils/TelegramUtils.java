...
import com.labshigh.aicfo.internal.api.common.utils.models.TelegramMessage;
...
public class TelegramUtils {
    ...
    @Value("${telegram.main_wallet_withdrawal_chat_id}")
    public String mainWalletWithdrawalChatId;
    @Value("${telegram.admin_tracking_chat_id}")
    public String adminTrackingChatId;
    ...
    /**
     * 텔레그램 메시지 전송(POST 방식 긴 내용) 입금 및 출금안내
     ...
     */
    public ResponseModel postSendOnMainWallet(String telegramMessage) throws InterruptedException {
        Thread.sleep(1000);
        log.info("[accessToken] : " + accessToken);
        log.info("[chatId] : " + mainWalletWithdrawalChatId);
        String url = "https://api.telegram.org/bot"+accessToken+"/sendMessage";
        // 요청 URL 세팅
        log.info("[Telegram Request Url] : " + url);
        ResponseModel responseModel = new ResponseModel();
        ...
        while(true) {
            ...
            try {
                ...
                log.info("[Telegram Request t_message] : " + telegramMessage);
                msg.setText(telegramMessage);
                msg.setChat_id(mainWalletWithdrawalChatId);
                String sendResult = WebClientHelper.getInstance().post(uri, msg);
                responseModel.setData(sendResult);
                return responseModel;
            } catch (Exception e) {
    ...
    public ResponseModel postSendOnAdminTracking(String telegramMessage) throws InterruptedException {
        Thread.sleep(1000);
        log.info("[accessToken] : " + accessToken);
        log.info("[chatId] : " + adminTrackingChatId);
        String url = "https://api.telegram.org/bot"+accessToken+"/sendMessage";
        // 요청 URL 세팅
        log.info("[Telegram Request Url] : " + url);
        ResponseModel responseModel = new ResponseModel();
        ...
        while(true) {
            ...
            try {
                URI uri = new URI(url);
                log.info("[Telegram Request Admin Tracking t_message] : " + telegramMessage);
                msg.setText(telegramMessage);
                msg.setChat_id(adminTrackingChatId);
                String sendResult = WebClientHelper.getInstance().post(uri, msg);
                responseModel.setData(sendResult);
                return responseModel;
            } catch (Exception e) {
